	...
	if [[ $(SLIM) == "true" ]]; then \
		...
		if [[ $(TARGET) == "aarch64-unknown-freebsd" ]]; then \
			...
			if [[ $(TARGET) == "aarch64-unknown-linux-musl" ]] || [[ $(TARGET) == "armv5te-unknown-linux-musleabi" ]]; then \
				RUSTFLAGS="-Zlocation-detail=none" cross +nightly build -Z build-std=std,panic_abort,core,alloc,proc_macro -Z build-std-features=panic_immediate_abort --target=$(TARGET) --release -- -C link-arg=-lgcc; \
			else \
